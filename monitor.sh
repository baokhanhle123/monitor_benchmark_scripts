...
# /proc/[pid]/status VmRSS (KB). All CPU% normalized to
# 0-100% of total machine capacity (nproc).
...
cmd_system() {
    local t1 i1 t2 i2 mtotal mavail
...
cmd_proc() {
    ...
    t2=$(now_ns)
    rss=$(read_proc_rss_kb "$pid") || rss=0
    awk -v c1="$c1" -v c2="$c2" -v t1="$t1" -v t2="$t2" \
        -v np="$NPROC" -v rss="$rss" \
        'BEGIN {
            dc = c2 - c1; dt = t2 - t1
            cpu = (dt > 0) ? (dc / (dt * np)) * 100 : 0
            printf "%.4f %d\n", cpu, rss
        }'
}
...
sample_pid() {
    ...
    c1=$(read_proc_cpu_ns "$pid") || { echo "PID $pid not found" >&2; exit 1; }
    t1=$(now_ns)
    echo "timestamp_ns,pid,cpu_pct,rss_kb"
    while true; do
        sleep "$interval"
        ...
        awk -v ts="$t2" -v pid="$pid" -v c1="$c1" -v c2="$c2" \
            -v t1="$t1" -v t2="$t2" -v np="$NPROC" -v rss="$rss" \
            'BEGIN {
                dc = c2 - c1; dt = t2 - t1
                cpu = (dt > 0) ? (dc / (dt * np)) * 100 : 0
                printf "%s,%s,%.4f,%d\n", ts, pid, cpu, rss
    ...
    while true; do
        ...
        while read -r pid; do
            ...
            awk -v ts="$curr_t" -v pid="$pid" -v c1="$c1" -v c2="$c2" \
                -v t1="$prev_t" -v t2="$curr_t" -v np="$NPROC" -v rss="$rss" \
                'BEGIN {
                    dc = c2 - c1; dt = t2 - t1
                    cpu = (dt > 0) ? (dc / (dt * np)) * 100 : 0
                    printf "%s,%s,%.4f,%d\n", ts, pid, cpu, rss
...
Subcommands:
  system                          One-shot: cpu_pct mem_pct (whole machine)
  pids                            List python PIDs (comm matches ^python[0-9.]*\$)
  proc <pid>                      One-shot: cpu_pct rss_kb for the given PID
  sample <interval_s> <target>    Continuous CSV to stdout (Ctrl+C to stop)
                                  target: <pid> | --system | --all-python

CPU% is normalized to 0-100% of total machine (nproc=$NPROC cores).
Per-process CPU is read from /proc/[pid]/schedstat (nanosecond resolution).
Memory is RSS in KB (per-process) or % of MemTotal (system).
EOF
}
